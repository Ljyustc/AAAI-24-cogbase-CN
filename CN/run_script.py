    ...
    if prompt == 0:  # 获取求解函数
        for i in range(num):
            ...
            while attempts < max_attempts:
                ...
                try:
                    file_index = f"{i:02}"
                    in_file = r"\data\input\GPT4_CN_PAL_4.json"
                    out_file = rf"\data\output\GPT4_CN_PAL_4_{file_index}.json"
                    # 运行内部脚本并等待其完成，默认执行中文数据集对应的脚本
                    subprocess.run(
        ...
        while attempts < max_attempts:
            print(f"Running the inner script: Attempt {attempts + 1}/{max_attempts}")
            try:
                in_file = r"\data\input\GPT4_CN_Verify.json"
                out_file = r"\data\output\GPT4_CN_Verify.json"
                # 运行内部脚本并等待其完成，默认执行中文数据集对应的脚本
                subprocess.run(
                    ['python', inner_script_path, '--in_file', in_file, '--out_file', out_file],
                    check=True)
        ...
        if attempts >= max_attempts:
            print(
                f"Reached the maximum number of attempts. Inner script did not complete successfully.")
